...
Remote backends (local, Modal, SSH) create sandboxes with no host files.
...
def get_cache_directory_mounts(
    ...
    Used by local to create bind mounts.  Each entry has ``host_path`` and
    ...
    for new_subpath, old_name in _CACHE_DIRS:
        ...
        if not host_dir.is_dir():
            # Create missing staging dirs instead of skipping them: local
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (local ``/root/.hermes``,
...
def from_agent_visible_cache_path(
    ...
    when the active backend is not local, or when the path is not under any
...
def to_agent_visible_cache_path(
    ...
    * local / modal — bind-mounted (local) or per-file-synced (modal) at
