    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"the Nous Research's FAL proxy. Either:\n"
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if env_name in {"localEnvironment", "SingularityEnvironment", "ModalEnvironment"}:
            return "/root/.hermes"
    ...
    if backend in {"local", "singularity", "modal"}:
        return "/root/.hermes"
    if backend == "ssh":
        return "~/.hermes"
    return None
...
def _confine_source_images(
    ...
    Under a non-local terminal backend (ssh/local/…), model-supplied local
