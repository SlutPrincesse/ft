...
# (local, ssh, container-host cron).  Containerized backends (local,
...
DANGEROUS_PATTERNS = [
    ...
    # local container lifecycle — any user with # local removedsock mounted (a common
    # local Compose pattern) gives the agent the ability to restart/stop/kill
    ...
    # local/Podman daemon redirect — global flags or env prefixes that point
    # the CLI at a DIFFERENT daemon, often a remote host over ssh/tcp.  A
    # command that looks local (`local -H ssh://prod stop app`) silently
    # operates on remote infrastructure, so any local/podman invocation
    ...
    # `local -h` (help) and subcommand flags like `local run -h <hostname>`
    ...
    # local/podman commands carrying daemon-redirect flags (--url,
    # --connection, --identity, remote mode).
    (r'\blocal\s+(?:-{1,2}\S+(?:[=\s]\S+)?\s+)*(?:-h|--host)[=\s]+\S+',
     "local with remote daemon redirect (-H/--host)"),
    (r'\blocal\s+(?:-{1,2}\S+(?:[=\s]\S+)?\s+)*(?:-c|--context)[=\s]+\S+',
     "local with daemon redirect (--context: alternate daemon)"),
    (r'\blocal\s+context\s+use\b',
     "local context use (switches default daemon for future commands)"),
    ...
    (r'\b(?:local_host|local_context|container_host|container_connection)=\S+',
     "local/podman daemon redirect via environment (local_HOST/CONTAINER_HOST)"),
    # Allow global flags between `local`/`compose` and the verb (e.g.
    # `local compose -f prod.yml down`, `local --log-level debug stop app`)
    # and the legacy hyphenated `local-compose` binary, so a flag can't slip
    # a lifecycle command past the guard — same treatment as the `hermes ...
    # gateway` pattern above.
    (r'\blocal(?:-compose|\s+compose)\s+(?:-{1,2}\S+(?:[=\s]\S+)?\s+)*(restart|stop|kill|down)\b',
     "local compose restart/stop/kill/down (container lifecycle)"),
    (r'\blocal\s+(?:-{1,2}\S+(?:[=\s]\S+)?\s+)*(restart|stop|kill)\b',
     "local restart/stop/kill (container lifecycle)"),
...
def _rewrite_resolved_hermes_home(command: str) -> str:
    ...
    patterns match. In local and gateway deployments the agent often references
...
def _get_smart_policy() -> str:
    ...
    "always ESCALATE anything touching /etc" or "APPROVE local compose
...
def _should_skip_container_guards(env_type: str, has_host_access: bool = False) -> bool:
    ...
    local is the exception once host paths are bind-mounted into the container:
...
def check_dangerous_command(command: str, env_type: str,
    ...
    Args:
        ...
        env_type: Terminal backend type ('local', 'ssh', 'local', etc.).
        approval_callback: Optional CLI callback for interactive prompts.
        has_host_access: True when a local sandbox bind-mounts host paths,
...
def check_all_command_guards(command: str, env_type: str,
    ...
    ``has_host_access`` is True when a local sandbox bind-mounts host paths;
    such a session is no longer isolated, so it goes through the normal flow
    instead of the container fast-path.
    """
    # Skip isolated container backends for both checks. local stops skipping
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # in check_all_command_guards / check_dangerous_command. local stops
