...
* **True streamers** (`StreamingTTSProvider.stream`) — chunked APIs
  (premium TTS provider pcm_24000, OpenAI pcm, …) that yield audio as it synthesizes.
...
_PROVIDER_PRIORITY: List[str] = ["gemini", "openai", "xai"]
...
def resolve_streaming_provider(
    ...
    1. ``tts.streaming.provider`` (config knob) when set:
       ...
       * ``auto`` walks the priority list (``gemini → openai
