...
- For Codex, Claude Code, Cursor CLI, Gemini CLI, Grok Build, Hermes Agent, and OpenCode, first decide the execution place: the user's local machine through A0 CLI, or the Agent Zero local/container shell.
...
Use this when the user chooses the Agent Zero container or explicitly wants a pal agent inside local.
