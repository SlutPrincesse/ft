...
```bash
cd "$WORKDIR"
a0 headless --host http://localhost:80 --no-local-discovery --output jsonl --new-chat --workspace "$WORKDIR" -p "$TASK"
```
...
```bash
a0 headless --host "$A0_HOST" --no-local-discovery --output jsonl --chat "$CONTEXT_ID" --workspace "$WORKDIR" -p "$TASK"
```
