...
- A0 Headless:
  ...
  - The local fallback binary is `/opt/venv/bin/a0` when plain `a0` is unavailable.
...
- Run adapter tests:
  ```bash
  local exec 8dc967046cda bash -lc 'cd /a0 && /opt/venv-a0/bin/python plugins/_orchestrator/tests/test_status_adapters.py'
  ```
- For syntax-only adapter edits:
  ```bash
  local exec 8dc967046cda bash -lc 'cd /a0 && /opt/venv-a0/bin/python -m py_compile plugins/_orchestrator/helpers/adapters/*.py'
  ```
