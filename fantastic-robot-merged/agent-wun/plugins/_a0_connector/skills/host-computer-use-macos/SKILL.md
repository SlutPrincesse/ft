...
Do not use this skill for Linux, Windows, Xpra, local, or browser-only tasks. If the backend is not macOS or does not advertise Accessibility-tree support, skip AX actions and follow the generic host computer-use rules.
...
Do not bypass a permission failure with server screenshots, local commands, `linux-desktop`, or browser fallbacks.
