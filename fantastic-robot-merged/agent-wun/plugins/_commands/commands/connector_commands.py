...
def _handle_browser(context: AgentContext | None, raw_args: str) -> dict[str, Any]:
    ...
    if action not in {"host", "container", "local"}:
    ...
    label = "Host browser through A0 CLI" if settings["runtime_backend"] == "host_required" else "Internal local browser"
...
def _browser_status(context: AgentContext | None) -> str:
    ...
    label = "Host browser through A0 CLI" if runtime == "host_required" else "Internal local browser"
