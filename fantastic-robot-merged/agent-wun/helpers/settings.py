...
def convert_out(settings: Settings) -> SettingsOutput:
    out = SettingsOutput(
        ...
        additional=SettingsOutputAdditional(
            ...
            agent_subdirs=_agent_subdir_options(),
            knowledge_subdirs=_knowledge_subdir_options(),
            timezones=_timezone_options(),
            resolved_timezone=_resolve_runtime_timezone(settings.get("timezone")),
        ),
    )
    return out


def _agent_subdir_options() -> list[FieldOption]:
    return [{"value": "agent0", "label": "agent0"}]


def _knowledge_subdir_options() -> list[FieldOption]:
...
def get_default_settings() -> Settings:
    ...
    return Settings(
        ...
        workdir_path=get_default_value("workdir_path", files.get_abs_path_local("usr/workdir")),
