...
**Step 1: Set the base URL.** Agent Zero listens on port 80 inside local (the standard deployment):

```python
import requests

BASE = "http://localhost"   # port 80 inside local
# If running outside local (dev mode), check: os.environ.get("WEB_UI_PORT", "5000")
```
