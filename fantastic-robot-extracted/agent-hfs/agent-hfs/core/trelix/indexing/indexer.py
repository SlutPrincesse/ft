...
from ..core.models import IndexedFile, Symbol
from ..embedder.base import BaseEmbedder, make_embedder
from ..indexing.chunker import Chunker, ContextualChunker
from ..indexing.parser.base import ParseResult
from ..indexing.parser.registry import get_parser
from ..indexing.walker import FileWalker
from ..store.db import Database
from ..store.vector import BaseVectorStore, make_vector_store
...
class Indexer:
    ...
    def __init__(
        ...
        try:
            from ..store.dimension_guard import DimensionGuard, DimensionMismatchError
    ...
    def _build_chunker(self, config: IndexConfig) -> Chunker:
        ...
        try:
            from ..llm.factory import build_chat_client
    ...
    def _build_file_summarizer(self, config: IndexConfig) -> object | None:
        ...
        if not config.file_summaries_enabled:
            return None
        try:
            from ..indexing.file_summarizer import FileSummarizer
            from ..llm.factory import build_chat_client
    ...
    def index(self) -> dict[str, Any]:
        ...
        if pending:
            try:
                from ..store.dimension_guard import DimensionGuard
        ...
        if self.config.retrieval.sparse_enabled and pending:
            try:
                from ..embedder.sparse import SparseEmbedder
                from ..store.sparse_store import SparseStore
    ...
    def _insert_one(self, pf: _ParsedFile, stats: dict[str, int]) -> list[_PendingChunk]:
        ...
        if self.config.parser.dataflow_enabled:
            try:
                from ..analysis.defuse import DataFlowExtractor
        ...
        if self.config.chunker.multi_granularity_enabled:
            try:
                from ..indexing.multi_granularity import (
                    Granularity,
    ...
    def _store_type_edges(
        ...
        """
        from ..core.models import TypeEdge
    ...
    def index_file(
        ...
        *,
        ...
        from ..indexing.walker import EXTENSION_MAP
