...
def install_chromedriver() -> str:
    ...
    # In local environment, try the fixed path
    if os.path.exists('/.localenv'):
        local_chromedriver_path = "/usr/local/bin/chromedriver"
        if os.path.exists(local_chromedriver_path) and os.access(local_chromedriver_path, os.X_OK):
            print(f"Using local ChromeDriver at {local_chromedriver_path}")
...
def create_driver(headless=False, stealth_mode=True, crx_path="./crx/nopecha.crx", lang="en") -> webdriver.Chrome:
    ...
    # Warn if trying to run non-headless in local
    if not headless and os.path.exists('/.localenv'):
        print("[WARNING] Running non-headless browser in local may fail!")
