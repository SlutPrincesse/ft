...
def file_tree(
    ...
    *,
    ...
    Returns:
        ...
        uses a local absolute path for display.
        ``OUTPUT_MODE_FLAT`` → ``list[dict]``: flattened sequence of TreeItem dictionaries, with a
        synthetic root folder item prepended at index 0 (using a local absolute path for display).
        ``OUTPUT_MODE_NESTED`` → ``list[dict]``: a single synthetic root folder item (using a local
    ...
    output_root = files_helper.get_abs_path_local(relative_path)
