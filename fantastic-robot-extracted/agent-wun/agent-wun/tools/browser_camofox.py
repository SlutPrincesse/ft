...
Setup::
    ...
    # Option 2: local
    local run -p 9377:9377 -e CAMOFOX_PORT=9377 jo-inc/camofox-browser

Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.hermes/.env``.
For local Camofox, optionally set ``CAMOFOX_REWRITE_LOOPBACK_URLS=true``
so page URLs like ``http://127.0.0.1:3000`` are opened inside the
container as ``http://host.# local removedinternal:3000``.
"""
...
def _loopback_rewrite_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    """Return whether loopback navigation URLs should be rewritten for local.

    ``CAMOFOX_URL`` itself often points at a host-published local port such as
    ...
    by the browser *inside* the local container.  In that context loopback
    points at the container, not the host running the web app.

    The rewrite is opt-in because non-local Camofox installs run the browser on
...
def _rewrite_loopback_url_for_camofox(url: str) -> tuple[str, Optional[Dict[str, str]]]:
    """Rewrite loopback page URLs for local-hosted Camofox, if configured.
...
def camofox_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        if rewrite_info:
            ...
            result["warning"] = (
                "Rewrote loopback URL for local-hosted Camofox: "
        ...
        return json.dumps({
            ...
            "error": f"Cannot connect to Camofox at {get_camofox_url()}. "
                     ...
                     "or: local run -p 9377:9377 -e CAMOFOX_PORT=9377 jo-inc/camofox-browser",
        })
