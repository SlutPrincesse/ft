...
Architecture (two transports):
  ...
  **Remote backends (file-based RPC):**
  ...
  3. Script runs inside the terminal backend (local/SSH/Modal/Daytona/etc.)
...
def _get_or_create_env(task_id: str):
    ...
    with task_lock:
        ...
        if env_type in {"local", "singularity", "modal", "daytona", "vercel_sandbox"}:
            container_config = {
...
def execute_code(
    ...
    from tools.terminal_tool import _get_env_config, _local_has_host_access
    ...
    # A local sandbox with host bind mounts is no longer isolated, so its
    # script does not get the container fast-path.
    from tools.approval import check_execute_code_guard
    _guard = check_execute_code_guard(
        code, env_type,
        has_host_access=_local_has_host_access(_env_config),
    )
