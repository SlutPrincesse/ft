...
# crashing in headless environments (SSH, local, WSL, no PortAudio).
# ---------------------------------------------------------------------------
...
def _import_mistral_client():
    ...
    directly). Mirrors the premium TTS provider lazy-import path.
    """
    try:
...
def _resolve_max_text_length(
    ...
    Resolution order:
      ...
      2. ``tts.providers.<provider>.max_text_length`` for user-declared
         command providers
      3. Model-aware character cap table (keyed on configured ``model_id``)
...
def _text_to_speech_single(
    ...
    *,
    ...
    if output_path:
        ...
        elif want_opus and provider in {"openai", "mistral", "gemini"}:
...
def stream_tts_to_speaker(
    ...
    Provider-agnostic. A registered streaming provider (OpenAI, …)
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            "text": {
                "type": "string",
                "description": "The text to convert to speech. Provider-specific per-request character caps apply automatically (OpenAI 4096, xAI 15000, MiniMax 10000); longer input is split into ordered chunks without silent truncation."
            },
            "output_path": {
                "type": "string",
            ...
            "provider": {
                ...
                "description": (
                    ...
                    "(edge, openai, minimax, xai, mistral, gemini, "
