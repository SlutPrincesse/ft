...
So a prompt-injected ``vision_analyze('/etc/passwd')`` under local reads the
...
def _ensure_container_env(task_id: Optional[str]) -> None:
    """Lazily bring up the sandbox (SSH/local/…) before an in-sandbox read.
...
async def _resolve_container_fallback(
    ...
    Cold-start retry: under local the very first exec against a freshly
    ...
    for attempt in range(2):
        ...
        if attempt == 0:
            ...
            # before retrying. 150ms covers local exec warm-up in practice
