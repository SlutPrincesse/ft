...
# probe external state (local daemon, Modal SDK install, playwright binary
...
# A single ``subprocess.run([local, "version"], timeout=5)`` that times out
...
def _check_fn_cached(fn: Callable) -> bool:
    ...
    re-probes) to keep flaky external checks (local daemon busy, socket
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        requirements probes modal/local, browser checks probe playwright,
