...
says not to use local/server/container execution.
...
- When the user gives a relative path like `tmp/file.txt`, keep it relative to
  ...
  Agent Zero server/local workdir, not the connected local terminal folder.
...
{
  "thoughts": [
    "The user asked for the connected local terminal rather than local, so I should execute on the A0 CLI host."
  ],
