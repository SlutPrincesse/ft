...
Use Codex for autonomous coding tasks in a repository. Inside the Agent Zero local runtime, the Codex sandbox may fail, so the local-safe default uses `--dangerously-bypass-approvals-and-sandbox`.
