...
After the user chooses, run only that command and relay its browser/device instructions. If the user gives an email to prefill, add `--email "<email>"`. Wait for confirmation, then retry the smoke prompt. Do not redirect them to a local shell just to choose a menu option. Do not start plain `claude` for login; it opens the first-run TUI (theme/provider menus) and is not a safe agent-driven login surface. Do not run bare `claude auth login` when provider choice is still unknown.
