...
- the user wants the agent to see the updated plugin without manually editing marketplace files
...
1. Update the plugin manifest to a single cachebuster suffix:
...
outside the agent depends on a specific token:
...
3. Reinstall from that marketplace name using the agent's plugin install command:

```bash
<agent plugin install command> <plugin-name>@<marketplace-name-from-marketplace-json>
```

The default personal marketplace is discovered implicitly from
`~/.agents/plugins/marketplace.json`. You do not need an explicit marketplace add command for that
path, and marketplace list commands are not the right check for whether that default
marketplace exists.
...
```bash
<agent plugin install command> <plugin-name>@<local-marketplace>
```

6. Prompt the user to use a new thread to try the updated plugin, so that the agent picks up new skills
   and tools.
...
Replace the existing cachebuster instead of appending another one. Do not keep incrementing
...
- Marketplace manipulation should happen through commands, not by hand-editing `marketplace.json`
  or agent config files during this update/reinstall flow.
...
- Read the personal marketplace name with
  ...
  `<agent plugin install command> <plugin-name>@<marketplace-name>`.
...
- Do not tell the user to run an explicit marketplace add command for the default personal-marketplace
  flow. That marketplace is discovered implicitly by the agent.
...
- Use the agent's plugin list command when the plugin lives in a different configured marketplace and you need
...
- If a non-default local marketplace has not been configured yet, install it with
  the agent's marketplace add command before telling the user to run
  `<agent plugin install command> <plugin-name>@<marketplace-name>`.
