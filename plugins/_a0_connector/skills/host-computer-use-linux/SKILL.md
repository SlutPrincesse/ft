...
Do not use this skill for macOS, Windows, Xpra, local, browser-only tasks, or the internal Agent Zero Desktop. If the backend is not Linux or does not advertise AT-SPI support, skip Linux structural actions and follow the generic host computer-use rules.
...
Do not bypass a permission or host-visibility failure with server screenshots, local commands, the built-in Linux Desktop/Xpra skill, or `code_execution_tool`.
