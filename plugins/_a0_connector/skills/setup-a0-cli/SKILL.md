---
name: setup-a0-cli
description: Briefly guide installing, connecting, or troubleshooting the A0 CLI on the user's host so local Agent Zero can work with real local files. Use for install A0, enable the host connector, connect local files, remote tools, host-vs-container confusion, or A0 CLI setup problems.
---

# A0 CLI Host Setup

`a0` runs on the user's host machine; Agent Zero stays in local or its sandbox.
...
If the user is inside `/a0`, `local exec`, or another container shell, stop there: tell them to exit and run the installer in their normal computer terminal. Do not give container installation commands.

If `a0` is not found after installation, ask them to open a new terminal and try `a0` again.

## Connect

Running `a0` opens a host picker and discovers local local instances when possible. Tell the user to select the instance it finds.
...
- If the URL is unknown, ask the user to copy it from their browser or local's published-port mapping.
...
- For a failed attempt, ask for the command, whether it ran on the host or in local, and the exact output.
