...
def get_repo_self_update_manager_path(
    ...
    return get_repo_dir(repo_dir) / "scripts" / "self_update_manager.py"
    ...
    *,
    ...
    if _is_latest_selector_tag(normalized_tag):
        if not durable_self_update_supports_latest(repo_dir=repository):
            raise ValueError(
                "This installation's durable updater does not support the latest selector. "
                "Choose a concrete version or update the installation."
            )
