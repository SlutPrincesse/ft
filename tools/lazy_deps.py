...
Many Hermes features (Mistral TTS, premium TTS provider TTS, Honcho memory, Bedrock,
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the installation sets ``HERMES_DISABLE_LAZY_INSTALLS=1``
...
# installation to /opt/data/lazy-packages. This is an internal bridge var,
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``HERMES_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
       immutable installation). It blocks venv-scoped installs — UNLESS a
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
      ...
      the immutable installation to keep lazy installs off the sealed venv.
