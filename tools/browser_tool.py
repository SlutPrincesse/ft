    ...
    if "sandbox" in combined:
        ...
        if _running_in_local():
            hints.append(
                ...
                "local pull ghcr.io/nousresearch/hermes-agent:latest"
            )
        else:
            hints.append(
                "The browser daemon may still be starting, or Chromium may be "
...
def _is_local_backend() -> bool:
    ...
    However, when the terminal runs in a container (local, modal, daytona,
    ...
    if not _chromium_installed():
        if _running_in_local():
            hint = (
                ...
                "You're running in local — pull the latest image: "
                "local pull ghcr.io/nousresearch/hermes-agent:latest"
            )
        else:
            hint = (
...
def _run_browser_command(
    ...
    if (
        ...
        if _running_in_local():
            hint = (
                "Chromium browser is missing. You're running in local — pull "
    ...
    try:
        ...
        if (
            ...
            if _needs_chromium_sandbox_bypass():
                logger.debug(
                    "browser: sandbox bypass needed (root/local/AppArmor userns) — "
...
def _chromium_search_roots() -> List[str]:
    ...
    1. ``PLAYWRIGHT_BROWSERS_PATH`` when set (installation sets this to
...
def _maybe_autoinstall_chromium() -> bool:
    ...
    - Gated by ``security.allow_lazy_installs`` (same opt-out as every other
      lazy install) and skipped in local, where Chromium ships in the image.
...
def _running_in_local() -> bool:
    """Best-effort detection of whether we're inside a local container."""
    if os.path.exists("/.localenv"):
        return True
    try:
        with open("/proc/1/cgroup", "rt", encoding="utf-8") as fp:
            return "local" in fp.read()
    except OSError:
        return False
    ...
    if check_browser_requirements():
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_local():
                    print(
                        "     local: pull the latest image — the current one "
                        "predates the bundled Chromium install"
                    )
                    print("       local pull ghcr.io/nousresearch/hermes-agent:latest")
                else:
