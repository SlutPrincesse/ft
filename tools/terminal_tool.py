...
A terminal tool that executes commands in local, local, Modal, SSH,
...
- "local": Execute in local containers (isolated, requires local)
- "modal": Execute in Modal cloud sandboxes (direct Modal or managed gateway)
- "vercel_sandbox": Execute in Vercel Sandbox cloud sandboxes

Features:
- Multiple execution backends (local, local, modal, vercel_sandbox)
...
def _local_volume_uses_host_path(volume_spec: str) -> bool:
    """Return True when a local volume spec bind-mounts a host path."""
...
def _local_has_host_access(config: Dict[str, Any]) -> bool:
    """Return True when a local sandbox exposes host paths through bind mounts."""
    if config.get("env_type") != "local":
        return False
    if config.get("host_cwd") and config.get("local_mount_cwd_to_workspace"):
        return True
    return any(_local_volume_uses_host_path(vol) for vol in config.get("local_volumes", []))
...
def _sudo_nopasswd_works() -> bool:
    ...
    Only probes for the `local` terminal backend; local/SSH/Modal/etc. must
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    Callers that drive a subprocess directly (local, ssh, local, singularity)
    ...
    # Scoped to the local terminal backend so local/SSH/Modal/etc. can't
...
# Once-per-process guard for the local orphan reaper (issue #20561).
# Set when _maybe_reap_local_orphans first runs; concurrent _create_environment
# calls for parallel subagents won't re-trigger the sweep.
_local_orphan_reaper_ran = False
_local_orphan_reaper_lock = threading.Lock()


def _maybe_reap_local_orphans(container_config: Dict[str, Any]) -> None:
    """Run the local orphan reaper once per process, if enabled.
    ...
    * ``terminal.local_orphan_reaper: false`` disables it entirely (the
      operator opted out — usually because they're running multiple
      Hermes processes in the same profile and don't trust the
      conservative defaults).
    * ``_local_orphan_reaper_ran`` flag — sweep runs once per Python
      interpreter, not on every subagent / RL-rollout / parallel
      ``terminal()`` call.
    """
    global _local_orphan_reaper_ran
    if not container_config.get("local_orphan_reaper", True):
        return
    # Cheap double-checked-locking: read without the lock, take the lock
    # only on first run, recheck inside.
    if _local_orphan_reaper_ran:
        return
    with _local_orphan_reaper_lock:
        if _local_orphan_reaper_ran:
            return
        _local_orphan_reaper_ran = True
    ...
    try:
        ...
        if removed:
            logger.info(
                "local orphan reaper removed %d stale container(s) for profile %s",
                removed, profile,
            )
    except Exception as e:
        # Never fail the env-creation path because of a janitor problem.
        logger.debug("local orphan reaper raised: %s", e)


# Per-task environment overrides registry.
# Allows environments (e.g., TerminalBench2Env) to specify a custom local/Modal
...
def register_task_env_overrides(task_id: str, overrides: Dict[str, Any]):
    ...
    per-task sandbox settings (e.g., a custom localfile for the Modal image).

    Supported override keys:
        - modal_image: str -- Path to localfile or local Hub image name
        - local_image: str -- installation name
...
# packages.  With per-session container isolation active (local +
...
def _local_session_isolation_enabled() -> bool:
    """True when local sessions get their OWN containers (issue: stale
    workspace mounts leaking between desktop sessions).

    Gated on ``terminal.backend: local`` + ``container_persistent: false``:
...
_ISOLATION_OVERRIDE_KEYS = frozenset({
    "local_image", "modal_image", "singularity_image",
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    per-task local/Modal image. When an override is registered for a
    ...
    Per-session container isolation (local + ``container_persistent:
    ...
    if task_id and _has_isolation_overrides(task_id):
        return task_id
    if task_id and _local_session_isolation_enabled():
...
def _resolve_task_host_cwd(config: Dict[str, Any], task_id: Optional[str]) -> Optional[str]:
    ...
    * Per-session isolation mode (local + ``container_persistent: false``):
    ...
    if config.get("env_type") != "local":
        return None
    if not config.get("local_mount_cwd_to_workspace"):
        return None
    if not _local_session_isolation_enabled():
...
_CONTAINER_BACKENDS = frozenset({"local", "singularity", "modal", "daytona", "vercel_sandbox"})
...
def _is_unusable_container_cwd(cwd: str) -> bool:
    ...
    ``local run -w`` and makes the container fail to start (exit 125).
    """
    if not cwd:
        return False
...
def _ensure_terminal_env_bridged() -> None:
    ...
    config.yaml selects ``terminal.backend: local``, running commands on the
...
def _get_env_config() -> Dict[str, Any]:
    ...
    mount_local_cwd = os.getenv("TERMINAL_local_MOUNT_CWD_TO_WORKSPACE", "false").lower() in {"true", "1", "yes"}
    container_backend = env_type in {"local", "singularity", "modal", "daytona", "vercel_sandbox"}
    local_backend = env_type == "local"

    # local/container-only env vars may be bridged from config.yaml even when
    ...
    # local value should not make local terminal/execute_code unusable.
    ...
    if local_backend:
        local_forward_env = _parse_env_var("TERMINAL_local_FORWARD_ENV", "[]", json.loads, "valid JSON")
        local_volumes = _parse_env_var("TERMINAL_local_VOLUMES", "[]", json.loads, "valid JSON")
        local_env = _parse_env_var("TERMINAL_local_ENV", "{}", json.loads, "valid JSON")
        local_extra_args = _parse_env_var("TERMINAL_local_EXTRA_ARGS", "[]", json.loads, "valid JSON")
        local_shm_size = os.getenv("TERMINAL_local_SHM_SIZE", "1g")
    else:
        local_forward_env = []
        local_volumes = []
        local_env = {}
    ...
    # If local cwd passthrough is explicitly enabled, remap the host path to
    ...
    host_cwd = None
    if env_type == "local" and mount_local_cwd:
        local_cwd_source = os.getenv("TERMINAL_CWD") or _safe_getcwd()
        candidate = os.path.abspath(os.path.expanduser(local_cwd_source))
        if (
    ...
    return {
        ...
        "local_image": os.getenv("TERMINAL_local_IMAGE", default_image),
        "local_forward_env": local_forward_env,
        "singularity_image": os.getenv("TERMINAL_SINGULARITY_IMAGE", f"local://{default_image}"),
        "modal_image": os.getenv("TERMINAL_MODAL_IMAGE", default_image),
        "daytona_image": os.getenv("TERMINAL_DAYTONA_IMAGE", default_image),
        "vercel_runtime": os.getenv("TERMINAL_VERCEL_RUNTIME", "").strip(),
        "cwd": cwd,
        "host_cwd": host_cwd,
        "local_mount_cwd_to_workspace": mount_local_cwd,
        ...
        # Container resource config (applies to local, singularity, modal,
        ...
        "local_env": local_env,
        "local_run_as_host_user": os.getenv("TERMINAL_local_RUN_AS_HOST_USER", "false").lower() in {"true", "1", "yes"},
        "local_network": os.getenv("TERMINAL_local_NETWORK", "true").lower() in {"true", "1", "yes"},
        "local_extra_args": local_extra_args,
        "local_shm_size": local_shm_size,
        ...
        # removed on exit).
        "local_persist_across_processes": os.getenv(
            "TERMINAL_local_PERSIST_ACROSS_PROCESSES", "true"
        ).lower() in {"true", "1", "yes"},
        ...
        "local_orphan_reaper": os.getenv(
            "TERMINAL_local_ORPHAN_REAPER", "true"
        ).lower() in {"true", "1", "yes"},
    }
...
def _container_config_from_config(config: Dict[str, Any]) -> dict:
    ...
    return {
        ...
        "local_volumes": config.get("local_volumes", []),
        "local_mount_cwd_to_workspace": config.get("local_mount_cwd_to_workspace", False),
        "local_forward_env": config.get("local_forward_env", []),
        "local_env": config.get("local_env", {}),
        "local_run_as_host_user": config.get("local_run_as_host_user", False),
        "local_extra_args": config.get("local_extra_args", []),
        "local_shm_size": config.get("local_shm_size", "1g"),
        "local_network": config.get("local_network", True),
        "local_persist_across_processes": config.get("local_persist_across_processes", True),
        "local_orphan_reaper": config.get("local_orphan_reaper", True),
    }
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    Args:
        env_type: One of "local", "local", "singularity", "modal",
            "daytona", "vercel_sandbox", "ssh"
        image: local/Singularity/Modal image name (ignored for local/ssh/vercel)
        ...
        host_cwd: Optional host working directory to bind into local when explicitly enabled
    ...
    volumes = cc.get("local_volumes", [])
    local_forward_env = cc.get("local_forward_env", [])
    local_env = cc.get("local_env", {})
    local_extra_args = cc.get("local_extra_args", [])
    local_network = cc.get("local_network", True)
    ...
    if env_type == "local":
        ...
        # Disable via ``terminal.local_orphan_reaper: false`` (issue #20561).
        _maybe_reap_local_orphans(cc)
        ...
        # existing lifecycle.
        session_scoped = (
            _local_session_isolation_enabled()
            and task_id != "default"
            and not _has_isolation_overrides(task_id)
        )
        local_env_obj = _localEnvironment(
            image=image, cwd=cwd, timeout=timeout,
            cpu=cpu, memory=memory, disk=disk,
            persistent_filesystem=persistent, task_id=task_id,
            volumes=volumes,
            host_cwd=host_cwd,
            auto_mount_cwd=cc.get("local_mount_cwd_to_workspace", False),
            forward_env=local_forward_env,
            env=local_env,
            run_as_host_user=cc.get("local_run_as_host_user", False),
            network=local_network,
            extra_args=local_extra_args,
            persist_across_processes=(
                False if session_scoped
                else cc.get("local_persist_across_processes", True)
            ),
            shm_size=cc.get("local_shm_size", "1g"),
        )
        ...
        # _localEnvironment may not accept attributes.
        if session_scoped:
            try:
                local_env_obj._session_scoped = True
            except AttributeError:
                pass
        return local_env_obj
        ...
        raise ValueError(
            f"Unknown environment type: {env_type}. Use 'local', 'local', "
...
def _cleanup_inactive_envs(lifetime_seconds: int = 300):
    ...
    # local teardown can block for 10-15s, which would stall every concurrent
    ...
    # are not blocked while Modal/local sandboxes shut down.
...
def ensure_task_env(task_id: Optional[str] = None):
    ...
    but nothing else did — so under a non-local backend (ssh, local, …) a
    session whose first action is ``vision_analyze`` on a container-only path hit
    "no active sandbox session" because the SSH/local handshake never ran
    ...
    overrides = resolve_task_overrides(task_id)
    if env_type == "local":
        image = overrides.get("local_image") or config["local_image"]
    elif env_type == "singularity":
...
def is_persistent_env(task_id: str) -> bool:
    ...
    point is to survive between turns (local with ``container_persistent``,
    ...
    Session-scoped local containers (per-session isolation mode) also count
...
def cleanup_vm(task_id: str, *, force_remove: bool = False):
    ...
    *force_remove* (default False) is forwarded to backends that accept it
    — currently only ``localEnvironment``. The default of False matches
    ...
    try:
        if hasattr(env, 'cleanup'):
            ...
            # (localEnvironment after issue #20561; other backends don't).
            import inspect
...
def _atexit_cleanup():
    ...
    if _active_environments:
        ...
        # the dict; we need them to wait on local cleanup threads after the
        # registry has been cleared.
        envs_to_wait = list(_active_environments.values())
        cleanup_all_environments()
        # Block briefly so local stop/rm actually completes before the
        # interpreter exits. Issue #20561 — without this join, the daemon
        # cleanup threads were getting torn down mid-`local stop`, leaving
...
_LONG_LIVED_FOREGROUND_PATTERNS = (
    ...
    re.compile(r"\blocal\s+compose\s+up\b", re.IGNORECASE),
...
def terminal_tool(
    ...
    try:
        ...
        # POSIX /home/<user>) reaches `local run -w <host-path>` and the
        ...
        if not force:
            approval = _check_all_guards(
                command, env_type,
                has_host_access=_local_has_host_access(config),
            )
        ...
        # Infrastructure/connection-class failure (SSH host down, local
...
def check_terminal_requirements() -> bool:
    ...
    try:
        ...
        if env_type == "local":
            ...
            from tools.environments.local import find_local
            local = find_local()
            if not local:
                logger.error("local executable not found in PATH or common install locations")
                return False
            result = subprocess.run([local, "version"], capture_output=True, timeout=5, stdin=subprocess.DEVNULL)
            ...
            logger.error(
                "Unknown TERMINAL_ENV '%s'. Use one of: local, local, singularity, "
...
if __name__ == "__main__":
    ...
    print(f"  installation: {config['local_image']}")
    ...
    print(
        ...
        "(local/local/singularity/modal/daytona/vercel_sandbox/ssh)"
    )
    print(f"  TERMINAL_local_IMAGE: {os.getenv('TERMINAL_local_IMAGE', default_img)}")
    print(f"  TERMINAL_SINGULARITY_IMAGE: {os.getenv('TERMINAL_SINGULARITY_IMAGE', f'local://{default_img}')}")
