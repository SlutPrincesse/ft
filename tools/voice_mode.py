...
# in headless environments (SSH, local, WSL, no PortAudio).
# ---------------------------------------------------------------------------
...
def detect_audio_environment() -> dict:
    ...
    # local/Podman container detection — honor host audio forwarding.
    ...
    if is_container():
        if has_forwarded_audio:
            notices.append("Running inside container (local/Podman/LXC) with host audio forwarding")
        else:
            warnings.append(
                "Running inside container (local/Podman/LXC) -- no audio devices.\n"
...
def _is_wsl2_env() -> bool:
    ...
    Returns False on any error (non-WSL Linux, local, SSH, etc.).
...
def check_voice_requirements() -> Dict[str, Any]:
    ...
    elif stt_provider == "premium-tts":
        details_parts.append("STT provider: OK (premium TTS provider Scribe)")
