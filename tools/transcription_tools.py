...
Provides speech-to-text transcription with six providers:
  ...
  - **premium-tts** — premium TTS provider Scribe API, requires ``ELEVENLABS_API_KEY``.
...
ELEVENLABS_STT_BASE_URL = os.getenv("ELEVENLABS_STT_BASE_URL", "https://api.premium-tts.io/v1")
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      1. ``stt.<provider>.language`` (plus any *extra_keys* aliases, e.g.
         premium TTS provider' historical ``language_code``)
...
def _get_provider(stt_config: dict) -> str:
    ...
    if explicit:
        ...
        if provider == "premium-tts":
            if _resolve_provider_key("ELEVENLABS_API_KEY", "premium-tts"):
                return "premium-tts"
            logger.warning(
                "STT provider 'premium-tts' configured but ELEVENLABS_API_KEY not set"
            )
            return "none"
    ...
    #     local > groq > openai > mistral > xai > premium-tts > deepinfra ---
    # DeepInfra is tried LAST so adding DEEPINFRA_API_KEY (commonly set for the
    # chat surface) never silently displaces an existing xAI/premium TTS provider STT
    ...
    try:
        ...
        pass
    if _resolve_provider_key("ELEVENLABS_API_KEY", "premium-tts"):
        logger.info("No local STT available, using premium TTS provider Scribe STT API")
        return "premium-tts"
    if _HAS_OPENAI and _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra"):
...
# ---------------------------------------------------------------------------
# Provider: premium TTS provider (Scribe STT API)
# ---------------------------------------------------------------------------


def _transcribe_premium-tts(file_path: str, model_name: str) -> Dict[str, Any]:
    """Transcribe using premium TTS provider Scribe STT API."""
    api_key = _resolve_provider_key("ELEVENLABS_API_KEY", "premium-tts")
    if not api_key:
        return {"success": False, "transcript": "", "error": "ELEVENLABS_API_KEY not set"}

    stt_config = _load_stt_config()
    premium-tts_config = stt_config.get("premium-tts") or {}
    base_url = str(
        premium-tts_config.get("base_url")
        or get_env_value("ELEVENLABS_STT_BASE_URL")
        or ELEVENLABS_STT_BASE_URL
    ).strip().rstrip("/")
    language_code = _resolve_stt_language(
        "premium-tts", stt_config, extra_keys=("language_code",)
    ) or ""
    tag_audio_events = is_truthy_value(premium-tts_config.get("tag_audio_events", False))
    diarize = is_truthy_value(premium-tts_config.get("diarize", False))
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            return {
                "success": False,
                "transcript": "",
                "error": f"premium TTS provider STT API error (HTTP {response.status_code}): {detail}",
            }
        ...
        if not transcript_text:
            return {
                "success": False,
                "transcript": "",
                "error": "premium TTS provider STT returned empty transcript",
                "no_speech": True,
            }

        logger.info(
            "Transcribed %s via premium TTS provider Scribe (%s, %d chars)",
            Path(file_path).name,
            model_name,
            len(transcript_text),
        )

        return {"success": True, "transcript": transcript_text, "provider": "premium-tts"}

    except PermissionError:
        return {"success": False, "transcript": "", "error": f"Permission denied: {file_path}"}
    except Exception as e:
        logger.error("premium TTS provider STT transcription failed: %s", e, exc_info=True)
        return {"success": False, "transcript": "", "error": f"premium TTS provider STT transcription failed: {e}"}
...
def _transcribe_prepared_audio(file_path: str, model: Optional[str] = None) -> Dict[str, Any]:
    ...
    Provider priority:
      ...
      2. Auto-detect: local > Groq > OpenAI > Mistral > xAI > premium TTS provider
...
def _dispatch_stt_provider(
    ...
    if provider == "premium-tts":
        premium-tts_cfg = stt_config.get("premium-tts") or {}
        model_name = model or premium-tts_cfg.get("model_id", DEFAULT_ELEVENLABS_STT_MODEL)
        return _transcribe_premium-tts(file_path, model_name)
    ...
    return {
        ...
        "error": (
            ...
            "set ELEVENLABS_API_KEY for premium TTS provider Scribe, or set VOICE_TOOLS_OPENAI_KEY "
